...
株取引ダッシュボードなど他のサービスも同居させる場合:

| 項目 | 推奨 | 備考 |
|------|------|------|
| モデル | **Raspberry Pi 5** | Pi 4比で2-3倍高速 |
| RAM | **8GB** | pandas/numpy等のデータ処理でメモリ消費大 |
| ストレージ | **USB SSD 必須** | SDカードは頻繁な書き込みで寿命が短くなる |
| 冷却 | ファン付きケース | 高負荷時のサーマルスロットリング防止 |

想定メモリ使用量:

| 用途 | メモリ |
|------|--------|
| Discord Bot | 100-300MB |
| 株ダッシュボード（Grafana/Dash等） | 200-500MB |
| データ収集/処理（pandas等） | 500MB-2GB+ |
| OS + バックグラウンド | 500MB-1GB |

> [!TIP]
> Bot単体なら4GBで十分。複数サービス同居なら8GB + USB SSD + Pi 5を推奨。
